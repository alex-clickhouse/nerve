...
# Fields that affect a job's scheduling or execution. Used by reload() to decide
# whether an existing job needs re-scheduling. Excludes:
#   * runtime-only bits (metadata, built gates, resolved prompt_path)
#   * cosmetic fields (description) — a label edit shouldn't reset a timer
#   * show_session_label — its effect is wired up in gateway/server.py at start()
#     (notification suppression), which reload() can't re-run, so it is
#     intentionally restart-only (documented in docs/cron.md).
# Excluded means "does not justify rebuilding the trigger", not "ignored":
# reload() swaps the rebuilt CronJob into the scheduler either way, so the
# excluded fields still reach the running job — they just don't reset its timer.
_JOB_COMPARE_FIELDS = (
    "schedule", "prompt", "prompt_file", "model", "effort", "cache_ttl",
    "session_mode", "context_rotate_hours", "context_rotate_at",
    "reminder_mode", "catchup", "enabled", "lock", "run_if", "skip_when_idle",
    "idle_consumer",
)


def _job_changed(old: CronJob, new: CronJob) -> bool:
    """True if any scheduling/execution-relevant field differs between two jobs."""
    return any(
        getattr(old, f) != getattr(new, f) for f in _JOB_COMPARE_FIELDS
    )
...
class CronService:
    ...
    async def reload(self) -> dict:
        ...
        daemon restart, mirroring the MCP-config reload pattern:
          ...
          * jobs added or re-enabled       → scheduled
          * changed jobs (schedule, prompt, gates, ...) → re-scheduled in place
        ...
        unregisters (see nerve/cron/gate_plugins.py). A job whose YAML is
        unchanged keeps its trigger but still has the rebuilt CronJob swapped
        into the scheduler, so gate edits reach the job that actually runs.
    ...
    async def _reload_locked(self) -> dict:
        ...
        # plugin's code is picked up, not just newly-added files.
        load_gate_plugins(self.config.cron.gate_plugins_dir, replace=True)
        ...
        # Added / re-enabled / changed jobs. The scheduler lookups here still
        # see the pre-reload schedule, which is what they are meant to compare
        # against; the two loops never visit the same id anyway (this one skips
        # disabled jobs and only walks ids present in the new config).
        for jid, job in new_by_id.items():
            if not job.enabled:
                continue
            old = old_by_id.get(jid)
            was_active = old is not None and old.enabled
            existing = self.scheduler.get_job(jid) is not None
            if was_active and not _job_changed(old, job):
                if existing:
                    # Nothing scheduling-relevant changed, so keep the trigger —
                    # re-adding would reset an interval job's timer. The stored
                    # CronJob still has to be swapped for the freshly built one:
                    # the scheduler runs the object it holds, and that object
                    # carries state _job_changed can't see — above all the gate
                    # instances, which are rebuilt from the just-reloaded plugin
                    # code and would otherwise keep evaluating the pre-edit class.
                    refresh.append(job)
                    continue
                # Enabled and unchanged, yet no trigger: something removed it
                # behind our back, or _jobs was refreshed from disk without the
                # scheduler. Skipping here would strand the job forever while
                # every reload keeps counting it as enabled, so rebuild it.
                logger.warning(
                    "Cron job '%s' is enabled but had no trigger — rescheduling",
                    jid,
                )
        ...
        for job in refresh:
            self.scheduler.modify_job(
                job.id, args=[job], name=job.description or job.id,
            )
