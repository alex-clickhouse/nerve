"""Asyncio helpers shared by the gateway and the services it supervises.
...
# asked, before falling back to cancellation. Sized for one cron reload (a few
# DB reads plus APScheduler mutations) or one external-agents sweep to finish,
# and short enough that a wedged task cannot hold the process open for a user
# watching it exit.
...
async def stop_background_task(
    ...
    of at the end of the cycle it was in. The cron watcher can be halfway
    through a ``reload()`` at that moment, which would leave the scheduler
    holding part of the new job set and part of the old; the external-agents
    sync can be halfway through a sweep, leaving some bundles rendered from
    the new sources and some from the old. So signal, wait a bounded moment
    for the task to wind itself down, and only cancel if it doesn't.
    """
